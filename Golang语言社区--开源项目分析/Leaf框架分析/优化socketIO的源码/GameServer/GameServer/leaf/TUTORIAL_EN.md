...
* Multi-core support. Leaf utilize its modules and [leaf/go](https://GameServer/leaf/tree/master/go) to make use of CPU resouces at maximum while avoiding varieties of side effects may be caused.
...
A game server implemented with Leaf may include many modules (e.g. [LeafServer](https://GameServer/leafserver)) which all share below traits:

* Each module runs inside a separate goroutine
* Modules communicate with one another via a light weight RPC channel([leaf/chanrpc](https://GameServer/leaf/tree/master/chanrpc))
...
[LeafServer](https://GameServer/leafserver) is a game server developped with Leaf. Let's start with it.

Download the source code of LeafServer：

```
git clone https://GameServer/leafserver
```

Download and install leafserver to GOPATH:

```
go get GameServer/leaf
```
...
More references are at [leaf/go](https://GameServer/leaf/blob/master/go)。
...
More references are at [leaf/timer](https://GameServer/leaf/blob/master/timer)。
...
More references are at [leaf/log](https://GameServer/leaf/blob/master/log).

### Leaf recordfile

Leaf recordfile is formatted in CSV([Example](https://GameServer/leaf/blob/master/recordfile/test.txt)). recordfile is to manage the configuration for game. The usage of recordfile in LeafServer is quite simple:
...
Refer to [leaf/recordfile](https://GameServer/leaf/blob/master/recordfile) for more details.

Learn more
----------

More references are at Wiki [https://GameServer/leaf/wiki](https://GameServer/leaf/wiki)
