...
int videoInput::getDeviceIDFromName(const char * name) {
	...
	for (int i = 0; i < VI_MAX_CAMERAS; i++) {
		if (std::string(deviceNames[i]) == name) {
			deviceID = i;
			break;
		}
	}
...
void videoInput::showSettingsWindow(int id, bool waitFor){
	...
	if(isDeviceSetup(id)){
		...
		if(hr == S_OK){
			...
			if(waitFor) {
				WaitForSingleObject(myTempThread, INFINITE);
			}
		}
	}
}
...
static bool setSizeAndSubtype(videoDevice * VD, int attemptWidth, int attemptHeight, GUID mediatype){
	...
	//store current size
	int tmpWidth  = pVih->bmiHeader.biWidth;
	int tmpHeight = pVih->bmiHeader.biHeight;
...
int videoInput::start(int deviceID, videoDevice *VD){
	...
	VIDEOINFOHEADER *pVih =  reinterpret_cast<VIDEOINFOHEADER*>(VD->pAmMediaType->pbFormat);
	int currentWidth	=  pVih->bmiHeader.biWidth;
	int currentHeight	=  pVih->bmiHeader.biHeight;
